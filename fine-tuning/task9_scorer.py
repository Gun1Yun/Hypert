...
def get_hypernyms(line, is_gold=True):
    if is_gold == True:
        valid_hyps = line.strip().encode().decode("utf-8").split("\t")
        return valid_hyps
    else:
        linesplit = line.strip().encode().decode("utf-8").split("\t")
        cand_hyps = []
        for hyp in linesplit[:15]:
            hyp_lower = hyp.lower()
...
if __name__ == "__main__":
    ...
    if len(args) == 2:
        ...
        fgold = open(gold, "r")
        fpredictions = open(predictions, "r", encoding="UTF8")

        goldls = fgold.readlines()
        predls = fpredictions.readlines()

        if len(goldls) != len(predls):
